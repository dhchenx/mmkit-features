...
root_path="medmnist_mmf"

mmf_file=f"{root_path}/adrenalmnist3d.mmf"
...
start_time=time.time()
birds_lib.to_index_file("text",f"{root_path}/text.index",index_type="brutal_force")
...
start_time=time.time()
birds_lib.to_index_file("text",f"{root_path}/text_inverted.index",index_type="inverted_index")
time_inverted=time.time()-start_time
list_result.append(("inverted indexing",time_inverted))

print("creating positional text...")
start_time=time.time()
birds_lib.to_index_file("text",f"{root_path}/text_positional.index",index_type="positional_index")
time_positional=time.time()-start_time
list_result.append(("positional indexing",time_positional))

# start to perform search test
query_str="lung cancer"

print("searching plain index test....")
start_time=time.time()
result_bf=birds_lib.search_index(index_file_path=f"{root_path}/text.index",query=query_str,search_type="brutal_force")
print(result_bf)
...
start_time=time.time()
result_bf=birds_lib.search_index(index_file_path=f"{root_path}/text_inverted.index",query=query_str,search_type="inverted_index")
print(result_bf)
...
start_time=time.time()
result_bf=birds_lib.search_index(index_file_path=f"{root_path}/text_positional.index",query=query_str,search_type="positional_index")
print(result_bf)
